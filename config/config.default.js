...
> 项目路径：{{path_with_namespace}}`,
      ...
      feishu: {
        push: `**<font color="green">{{user_name}}</font>** {{GB_op}} [[{{project.name}} | {{GB_branch}}分支]({{project.web_url}}/tree/{{GB_branch}})]
> 包含**<font color="green">{{total_commits_count}}</font>**个提交，新增**<font color="green">{{GB_changes.added}}</font>** | 修改**<font color="green">{{GB_changes.modified}}</font>** | 删除**<font color="green">{{GB_changes.removed}}</font>**
{{#commits}}
...
`,
        ...
        pipeline: `[[#{{GB_pipelineId}}流水线 | {{object_attributes.ref}}分支]({{GB_pipelineUrl}})] <font color="{{GB_status.color}}">{{GB_status.str}}</font>**，由**<font color="green">{{user.name}}</font>**通过**<font color="green">{{GB_sourceString}}</font>**触发。
...
> {{#merge_request}}**合并详情：** [{{title}}]({{url}})，**<font color="green">{{source_branch}}</font>**合并至**<font color="green">{{target_branch}}**<font color="green">{{/merge_request}}
> {{#commit}}**提交详情：** **<font color="green">{{author.name}}</font>**： [{{message}}]({{url}}){{/commit}}
> **编译详情**： 
{{#GB_builds}}> 》 **<font color="green">{{stage}}</font>**： 
{{#builds}}> - [**<font color="green">{{name}}**<font color="green">{{#GB_duration}}({{GB_duration}}){{/GB_duration}} -> <font color="{{GB_status.color}}">{{GB_status.str}}{{#failure_reason}}，{{failure_reason}}{{/failure_reason}}</font>** {{#GB_user}}({{GB_user}}){{/GB_user}}]({{GB_url}})
{{/builds}}
{{/GB_builds}}
...
`,
        ...
        merge_request: `{{#GB_stateAction}}{{GB_stateAction}} ：{{/GB_stateAction}}**<font color="green">{{user.name}}**<font color="green">{{GB_stateString}}{{#object_attributes}}**[[#{{iid}}合并请求 {{title}}]({{iid}})]，从**<font color="green">{{source_branch}}</font>**合并至**<font color="green">{{target_branch}}**<font color="green">{{/object_attributes}}
> **MR详情：**
> 提交时间： {{GB_updated_at}}
> 提交详情： 
> {{#object_attributes.last_commit}}》 {{author.name}}： [{{title}}]({{url}}){{/object_attributes.last_commit}}
...
{{#project}}项目信息:[[{{name}} / {{namespace}}]({{web_url}})]{{/project}}
`,
        tag_push: `**<font color="green">{{user_name}}**<font color="green">{{GB_op}}标签 [[{{project.name}} | {{GB_tag}}]({{web_url}}/-/tags/{{GB_tag}})]。
> 包含**<font color="green">{{total_commits_count}}</font>**个提交，**<font color="green">{{GB_changes.added}}</font>**新增 | **<font color="green">{{GB_changes.modified}}</font>**修改 | **<font color="green">{{GB_changes.removed}}</font>**删除
{{#commits}}
> 》 **<font color="green">{{author.name}}</font>**： [{{title}}]({{url}})
{{/commits}}
`,
        issue: `**<font color="green">{{user.name}}</font>**<font color="{{GB_state.color}}">{{GB_state.str}}</font>** {{#object_attributes}}[[#{{id}}议题]({{url}})]{{/object_attributes}}
> **议题详情：**
{{#object_attributes}}> 标题： [{{title}}]({{url}})
> 描述： {{description}}
> 动作： {{action}}{{/object_attributes}}
...
{{#project}}项目信息： [[{{name}} / {{namespace}}]({{web_url}})]{{/project}}
`,
        wiki: `**<font color="green">{{user.name}}</font>** {{#object_attributes}}<font color="{{GB_action.actionColor}}">{{GB_action.actionString}}</font>** WIKI页 [{{title}}](url)
> 内容： {{content}}{{/object_attributes}}

{{#project}}项目信息： [[{{name}} / {{namespace}}]({{web_url}})]{{/project}}
`,
        note: `**<font color="green">{{user.name}}</font>** 评论了 {{#object_attributes}} [{{note}}](url)
> **评论详情：**
> 类型： **<font color="green">{{noteable_type}}**<font color="green">{{/object_attributes}}
{{#merge_request}}> 标题： [{{title}}]({{url}})
> 分支： 从**<font color="green">{{source_branch}}</font>**合并至**<font color="green">{{target_branch}}</font>**
> 责任人： **<font color="green">{{assignee.name}}**<font color="green">{{/merge_request}}
{{#issue}}> 标题： [{{title}}]({{url}})
> 状态： **<font color="green">{{state}}**<font color="green">{{/issue}}
{{#commit}}> 提交： {{author.name}} ： [{{message}}]({{url}}){{/commit}}
{{#snippet}}> 标题： [{{title}}]({{url}})
文件： {{file_name}}{{/snippet}}
